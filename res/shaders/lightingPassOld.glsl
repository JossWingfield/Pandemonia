...
void main()
{
    ...
    for(int i = 0; i < uNumLights; i++)
    {
        ...
        if(uShadowsEnabled)
        {
            ...
            {
                vec2 left =
                    uBuildingCasterLeft[s];

                vec2 right =
                    uBuildingCasterRight[s];

                vec2 edgeMid =
                    (left + right) * 0.5;

                vec2 dirL =
                    normalize(left - L.position);

                vec2 dirR =
                    normalize(right - L.position);

                float projectionLength =
                    (L.radius -
                    length(edgeMid - L.position))
                    * uBuildingShadowLengthMultiplier;

                projectionLength =
                    max(projectionLength, 1.0);

                vec2 farLeft =
                    left + dirL * projectionLength;

                vec2 farRight =
                    right + dirR * projectionLength;
                ...
                if(inside)
                {
                    ...
                    vec2 farCenter =
                        (farLeft + farRight) * 0.5;

                    vec2 shadowAxis =
                        normalize(farCenter - nearCenter);

                    float forwardDist =
                        dot(fragPos - nearCenter, shadowAxis);

                    float fade =
                        1.0 - (forwardDist / projectionLength);
            ...
            for(int s = 0; s < uNumPlayerCasters; s++)
            {
                ...
                float casterDist =
                    length(casterPos - L.position);

                if(casterDist < L.radius * 3.0)
                {
                    vec2 shadowDir =
                        normalize(casterPos - L.position);

                    vec2 fragToCaster =
                        fragPos - casterPos;

                    float angle =
                        atan(shadowDir.y, shadowDir.x);
                    ...
                    if(shadowSpace.x > 0.0)
                    {
                        float dynamicLength =
                            casterSize.x *
                            (1.0 +
                            (L.radius - casterDist) / L.radius);

                        float sx =
                            shadowSpace.x / dynamicLength;

                        float sy =
                            shadowSpace.y / casterSize.y;

                        float shadow =
                            1.0 - length(vec2(sx, sy));

                        shadow =
                            clamp(shadow, 0.0, 1.0);

                        shadow =
                            smoothstep(0.0, 1.0, shadow);

                        shadowMask *=
                            1.0 -
                            shadow *
                            uPlayerCasterStrength[s];
                    }
                }
            }
        }

        lighting += lightContribution * shadowMask;
    }
